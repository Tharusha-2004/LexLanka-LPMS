<x-layouts.app>
    ...
    <x-slot name="header">Open New Case</x-slot>
    ...
    <div class="max-w-2xl">
        <div class="card">
            <form method="POST" action="{{ route('cases.store') }}" novalidate>
                @csrf
                ...
                <div class="space-y-5">
                        ...
                        <select id="client_id" name="client_id"
                            ...
                            @foreach ($clients as $client)
                                <option value="{{ $client->id }}"
                                    {{ old('client_id', $preselectedClientId) == $client->id ? 'selected' : '' }}>
                                    {{ $client->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select id="assigned_attorney_id" name="assigned_attorney_id"
                            ...
                            @foreach ($attorneys as $attorney)
                                <option value="{{ $attorney->id }}"
                                    {{ old('assigned_attorney_id') == $attorney->id ? 'selected' : '' }}>
                                    {{ $attorney->name }}
                                    ({{ ucfirst($attorney->role) }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input id="case_type" type="text" name="case_type"
                               value="{{ old('case_type') }}"
                               placeholder="e.g. Civil Litigation, Property Dispute, Criminal Defence…"
                               class="w-full px-3.5 py-2.5 text-sm rounded-lg border
